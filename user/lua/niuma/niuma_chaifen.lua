...
local processor = rime.make_option_cycler(options,
  'niuma_chaifen/lua/cycle_key',
  'niuma_chaifen/lua/switch_key')
...
local function get_tricomment(cand, env)
  ...
  if utf8.len(text) == 1 then
    ...
    return env.engine.context:get_option('niuma_chaifen.lv1')
        and xform(raw_spelling:gsub('%[(.-),.*%]', '[%1]'))
        or env.engine.context:get_option('niuma_chaifen.lv2')
        and xform(raw_spelling:gsub('%[(.-,.-),.*%]', '[%1]'))
        or xform(raw_spelling) -- niuma_chaifen.lv3 is on
  elseif utf8.len(text) > 1 then
    ...
    if env.engine.context:get_option('niuma_chaifen.lv1') then
...
local function filter(input, env)
  if env.engine.context:get_option('niuma_chaifen.off') then
